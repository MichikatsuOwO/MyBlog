  ...
  return <section className="editor-shell">
    ...
    <form className="editor writer-layout project-editor" onSubmit={save}>
      <div className="editor-main writer-canvas">
        ...
        <div className="content-header"><span>项目详情（Markdown）</span><div><button type="button" className="text-button demo-upload" onClick={() => demoInput.current?.click()}>上传 Demo 图</button><button type="button" className="preview-button" onClick={() => setPreview(!preview)}>{preview ? "返回编辑" : "预览详情"}</button></div></div>
        <input ref={demoInput} className="file-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void uploadDemo(event.target.files?.[0])} />
        {preview ? <div className="markdown-preview project-markdown-preview"><Markdown content={project.content || "## 项目详情\n\n从这里补充项目背景、核心功能、技术方案和 Demo 图。"} /></div> : <><div className="markdown-toolbar"><button type="button" onClick={() => insert("## ", "", "项目背景")}>H2</button><button type="button" onClick={() => insert("### ", "", "核心功能")}>H3</button><button type="button" onClick={() => insert("**", "**", "加粗文字")}>B</button><button type="button" onClick={() => insert("[", "](https://)", "链接文字")}>链接</button><button type="button" onClick={() => demoInput.current?.click()}>图片</button></div><textarea ref={contentInput} className="content writer-content project-detail-input" placeholder={"## 项目背景\n\n说明你要解决的问题。\n\n## 核心功能\n\n- 功能一\n- 功能二\n\n## Demo 展示\n\n点击“上传 Demo 图”后会自动插入图片。"} value={project.content} onChange={(event) => update({ content: event.target.value })} /></>}
        {uploading && <p className="project-upload-notice">{uploading}</p>}
      </div>
      <aside className="publish-panel"><h2>项目设置</h2><label>状态<select value={project.status} onChange={(event) => update({ status: event.target.value as Status })}><option value="draft">草稿</option><option value="published">发布</option><option value="hidden">隐藏</option></select></label><label>技术标签<input placeholder="例如：Next.js, PostgreSQL" value={project.tags.join(", ")} onChange={(event) => update({ tags: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })} /></label><label>线上 Demo / 仓库链接<input placeholder="https://..." value={project.url} onChange={(event) => update({ url: event.target.value })} /></label><button disabled={loading || Boolean(uploading)}>{loading ? "正在保存…" : project.status === "published" ? "发布项目" : "保存草稿"}</button></aside>
    </form>
  </section>
}
